...
func knockMultipleTimes(amnt: Int)
{
    for _ in 0..<amnt
    {
        knockknock()
...
func calculateAge(yearOfBirth: Int) -> Int
{
    let currentDate = NSDate()
    let currentCalendar = NSCalendar.currentCalendar()
    let currentYear = currentCalendar.components(NSCalendarUnit.Year, fromDate: currentDate)
    let age = currentYear.year - yearOfBirth
    return age
}
calculateAge(1983)
...
// Call replyWhoIsHere
replyWhoIsHere("Keith", homeCountry: "USA")

// Call tellNameAndAge
tellNameAndAge("Keith", yearOfBirth: 1983)
