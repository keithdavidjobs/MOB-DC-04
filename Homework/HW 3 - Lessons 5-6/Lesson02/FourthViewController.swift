...
*/
    ...
    {
        //code
        if let tempNum = Int(self.fibNum.text!)
        {
            if tempNum > 0
            {
                var tempFib = FibonacciAdder(seq: tempNum)
                var tempFinalNum = tempFib.fibonacciNumberAtIndex(tempNum)
                self.resultField.text = String(tempFinalNum)
            }
        }
    }
}
